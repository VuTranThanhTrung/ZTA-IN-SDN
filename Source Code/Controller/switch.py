...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    def switch_features_handler(self, ev):
        ...
        # Xóa tất cả các flows và meters cũ từ phiên làm việc trước
        self.logger.info("Switch (Datapath ID: %016x) kết nối. Đang xóa các flows và meters cũ...", datapath.id)
        self.delete_all_flows(datapath)
        self.delete_all_meters(datapath)

        # Cài đặt luật table-miss mặc định để gửi các gói tin chưa khớp về Controller
    ...
    def delete_all_meters(self, datapath):
        ofproto = datapath.ofproto
        parser = datapath.ofproto_parser
        ...
        if out_port != ofproto.OFPP_FLOOD:
            ...
            if eth.ethertype == ether_types.ETH_TYPE_IP:
                ...
                if msg.buffer_id != ofproto.OFP_NO_BUFFER:
                    ...
                    self.add_flow(datapath, 1, match, actions, idle=10, hard=50)
        data = None
        if msg.buffer_id == ofproto.OFP_NO_BUFFER:
            data = msg.data
